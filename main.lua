...
if not script_dir then
    mp.msg.error("Install this to its own directory. (READMEs are there to be, y'know, read.)")
    return
end
...
if ffi.abi("32bit") then
    mp.msg.error("This script probably won't work with 32-bit Windows in its current state. Bailing out.")
    return
end
...
local icons = {
    ...
    [C.BUTTON_PLAY_PAUSE] = {
        [false] = script_dir .. "\\res\\light-pause.ico", -- *shrug*
...
function on_pl_pos_change(_, value)
    ...
    for k, v in pairs(cmp_values) do
        ...
        if value == v then
            if bit.band(buttons[k].dwFlags, C.THBF_DISABLED) ~= C.THBF_DISABLED then
                buttons[k].dwFlags = bit.bor(C.THBF_DISABLED, C.THBF_NOBACKGROUND)
...
function on_vo_configured(_, value)
